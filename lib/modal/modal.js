...
(function() {
    ...
    Modal.prototype.open = function () {
        ...
        if (isMobile(navigator.userAgent)) {
            this.modal.className = 'focalhot-modal animated ' + this.settings.animationIn;
        } else {
    ...
    Modal.prototype.close = function () {
        var that = this;
        this.modal.className = this.modal.className.replace(that.settings.animationIn, that.settings.animationOut);
        if (isMobile(navigator.userAgent)) {
            addHandler(that.modal, 'animationend', function () {
                document.body.removeChild(that.modal);
                document.body.removeChild(that.overlay);
            });
            addHandler(that.modal, 'webkitAnimationEnd', function () {
                document.body.removeChild(that.modal);
                document.body.removeChild(that.overlay);
            });
